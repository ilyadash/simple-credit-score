...
class MyDataPreprocessor(TransformerMixin):
    ...
    def fit(self, data: pd.DataFrame, *args):
        ...
        """
        data = data.drop_duplicates()
        self.person_age_median = data['person_age'].median(skipna=True)
        self.person_emp_length_median = data['person_emp_length'].median(skipna=True)
        ...
        if self.categorial_columns is not None:
